...
// store collective workload
map<int, MsgEvent> collective_workload_map;
// Helper functions for collective workload management
void register_collective_workload(int src_id, int dst_id, int totalPacketCount,
                                  void (*msg_handler)(void *fun_arg), int tag) {
  // Store the collective workload
  MsgEvent collective_event(src_id, dst_id, 0, totalPacketCount, nullptr, msg_handler);
  collective_workload_map[tag] = collective_event;
}

MsgEvent get_collective_workload(int tag) {
  if (collective_workload_map.find(tag) != collective_workload_map.end()) {
    return collective_workload_map[tag];
  } else {
    std::cerr << "Error: No collective workload found for tag " << tag << std::endl;
    return MsgEvent(); // Return default MsgEvent
  }
}

void process_collective_workload(int tag) {
  MsgEvent event = get_collective_workload(tag);
  if (event.msg_handler) {
    event.callHandler();
  }
}

void remove_collective_workload(int tag) {
  if (collective_workload_map.find(tag) != collective_workload_map.end()) {
    collective_workload_map.erase(tag);
  } else {
    std::cerr << "Error: No collective workload found for tag " << tag << std::endl;
  }
}
...
void send_flow_collective(int src_id, int dst_id, int totalPacketCount,
                          void (*msg_handler)(void *fun_arg), void *fun_arg, int tag) {
  // Create and store the collective workload
  register_collective_workload(src_id, dst_id, totalPacketCount, msg_handler, tag);
}
...
int setup_ns3_simulation(string network_configuration) {
  ...
  if (SetupNetwork(qp_finish)) {
    // Specify the output files for routing table and flow paths
    const std::string routingTableFile = "routing_table.txt";
    const std::string flowPathsFile = "flow_paths.txt";
    generateRoutingAndFlowPaths(routingTableFile, flowPathsFile);
  } else {
    return -1;
  }
