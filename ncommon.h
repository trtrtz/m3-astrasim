...
std::map<std::pair<uint32_t, uint32_t>, std::vector<uint32_t>> flowPaths;
struct FlowInput {
...
void CalculateRoute(Ptr<Node> host) {
    ...
    delay[host] = 0;
    txDelay[host] = 0;
    bw[host] = 0xfffffffffffffffflu;  // Max bandwidth
    
    // BFS traversal.
    while (!q.empty()) {
        Ptr<Node> now = q.front();
        q.pop();
        int d = dis[now];
        ...
        for (auto it = nbr2if[now].begin(); it != nbr2if[now].end(); it++) {
            ...
            if (dis.find(next) == dis.end()) {
                ...
                if (next->GetNodeType() == 1) {
                    q.push(next);  // Push switch nodes if applicable
                }
            }
            
            // Store next hop for shortest path.
            if (d + 1 == dis[next]) {
    ...
    // Update delay, txDelay, bandwidth, RTT, and BDP.
    for (auto it : delay) {
    ...
    // Calculate RTT and BDP for each node pair.
    for (auto it : delay) {
        Ptr<Node> dest = it.first;
        uint64_t nodeDelay = it.second;
        rtt[dest] = 2 * nodeDelay;  // RTT is typically 2 times the delay
        bdp[dest] = rtt[dest] * bw[dest];  // BDP = RTT * Bandwidth
        
        pairRtt[host->GetId()][dest->GetId()] = rtt[dest];
        pairBdp[host][dest] = bdp[dest];
    }
}
...
void CalculateRoutes(NodeContainer &n) {
    for (uint32_t i = 0; i < n.GetN(); i++) {
        ...
        if (node->GetNodeType() == 0) {  // Host node type
            CalculateRoute(node);
        }
    }
}

void generateRoutingAndFlowPaths(const std::string &routingTableFile, const std::string &flowPathsFile) {
  // Open the file for routing table
  std::ofstream routingOut(routingTableFile);
  if (!routingOut.is_open()) {
    std::cerr << "Error: Cannot open file for writing routing table: " << routingTableFile << std::endl;
    return;
  }

  // Write Routing Table
  routingOut << "Routing Tables:\n";
  for (auto &srcEntry : nextHop) {
    Ptr<Node> src = srcEntry.first;
    routingOut << "Routing Table for Node " << src->GetId() << ":\n";
    for (auto &dstEntry : srcEntry.second) {
      Ptr<Node> dst = dstEntry.first;
      for (auto &next : dstEntry.second) {
        uint32_t interface = nbr2if[src][next].idx;
        routingOut << "  Destination: " << dst->GetId()
                   << ", Next Hop: " << next->GetId()
                   << ", Interface: " << interface << "\n";
      }
    }
  ...
  routingOut.close();
  std::cout << "Routing table written to " << routingTableFile << std::endl;

  // Open the file for flow paths
  std::ofstream flowPathsOut(flowPathsFile);
  if (!flowPathsOut.is_open()) {
    std::cerr << "Error: Cannot open file for writing flow paths: " << flowPathsFile << std::endl;
    return;
  }

  // Write Flow Paths
  flowPathsOut << "Flow Paths:\n";
  for (auto &dstEntry : nextHop) {
    Ptr<Node> dst = dstEntry.first;
    for (auto &srcEntry : dstEntry.second) {
      Ptr<Node> src = srcEntry.first;
      std::vector<uint32_t> path;

      // Construct the path
      Ptr<Node> current = dst;
      while (current != src) {
        path.push_back(current->GetId());
        if (nextHop[current][src].empty()) {
          std::cerr << "No path found from " << src->GetId() << " to " << dst->GetId() << std::endl;
          break;
        }
        current = nextHop[current][src][0];
      }
      path.push_back(src->GetId());
      std::reverse(path.begin(), path.end());

      // Write the path to the file
      flowPathsOut << "Source: " << src->GetId()
                   << ", Destination: " << dst->GetId()
                   << ", Path: ";
  ...
  flowPathsOut.close();
  std::cout << "Flow paths written to " << flowPathsFile << std::endl;
}
...
void SetRoutingEntries() {
    for (auto i = nextHop.begin(); i != nextHop.end(); i++) {
        ...
        for (auto j = table.begin(); j != table.end(); j++) {
            ...
            for (uint32_t k = 0; k < nexts.size(); k++) {
                Ptr<Node> next = nexts[k];
                uint32_t interface = nbr2if[node][next].idx;  // Interface index
                
                if (node->GetNodeType() == 1) {
                    // For switch nodes, add routing entry
                    DynamicCast<SwitchNode>(node)->AddTableEntry(dstAddr, interface);
                } else {
                    // For host nodes, use RDMA
...
bool SetupNetwork(void (*qp_finish)(FILE *, Ptr<RdmaQueuePair>)) {
  ...
  // setup routing
  //Extract routing table after route calculation
  CalculateRoutes(n);

  //proceed with setting route entries
  SetRoutingEntries();
